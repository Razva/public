...
An active state normally requires the exact recorded File 2 revision and verification against its stored digest. Never substitute the current File 2 or a remembered copy after File 2's one-time use.

If the exact recorded revision is no longer retrievable from its canonical repository or an approved immutable archive, the owner may provide a complete recovery byte source. Use it only when a qualifying tool hashes those exact bytes, the result equals the lifecycle's stored digest, and completeness through end-of-file is established. This recovers the original recorded File 2; it is not a second bootstrap supply and grants nothing beyond that revision. If no matching recovery source exists, fail closed for resumed mutation and ask the owner for a separate explicit lifecycle-recovery decision; do not alter lifecycle state, infer approval, or choose a replacement workflow silently.

When the exact immutable File 2 revision and its complete canonical content are established but no qualifying hashing tool is available, report the limitation and ask the owner whether to authorize the specific recorded bootstrap stage under that exact revision. Such approval waives only the missing-digest gate for that stage; it does not permit a different, current, remembered, or unverified supplied File 2, infer any product or proposal approval, repair ambiguous lifecycle evidence, or broaden bootstrap scope. Without that explicit approval, continue only safe read-only or unaffected work.
