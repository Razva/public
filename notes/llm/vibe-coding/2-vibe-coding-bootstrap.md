...
Reusable technology profiles are conditional technical references, never additional workflows. This section is the agent-facing profile registry: every maintained profile must be linked here, and the people-facing README is not a substitute. When a listed technology is required by an owner constraint, detected in preserved project material, or becomes a credible candidate for the recommended baseline, retrieve its canonical profile completely at an exact revision before finalizing the recommendation. When TypeScript is applicable, retrieve the [TypeScript Project Profile](https://github.com/Razva/public/blob/main/notes/llm/vibe-coding/profiles/typescript.md); apply its conditional Next.js guidance only when Next.js is independently required, detected, or a credible candidate.
