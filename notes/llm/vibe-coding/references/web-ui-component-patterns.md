...
This file is a reusable, language-neutral reference for product-quality web UI primitives. The [Vibe-Coded Project Bootstrap](../2-vibe-coding-bootstrap.md) retrieves it when the proposed product includes a user interface, selects only web-specific patterns relevant to the product's surfaces and stack, records any accepted project-specific behaviour in the application's local UI documentation, and then treats that project-local contract as canonical.
